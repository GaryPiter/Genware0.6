...
import com.genware.activity.AddFriendActivity;
import com.genware.activity.GroupChatActivity;
import com.genware.activity.R;
import com.genware.activity.UpdateActivity;
...
public class XXApp extends Application {
	...
	public static XMPPConnection mConn;
	 ...
	 * PopupWindow选项事件
	 */
	public static void showPopupWindow(final Context context,ImageView addItemBtn) {
		// 创建popupwindow对话框
		 popupwindow = new PopupWindow(context);
		// 得到布局加载器
		LayoutInflater inflater = LayoutInflater.from(context);
		// 加载布局文件
		View contentView = inflater.inflate(R.layout.custom_commen_title_rightview, null);
		popupwindow.setContentView(contentView);
		// popupWindow控件设置点击事件
		contentView.findViewById(R.id.pop_add_friend).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				//搜索好友
				Intent intentAdd=new Intent(mApplication, AddFriendActivity.class);
				context.startActivity(intentAdd);
				popupwindow.dismiss();
			}
		});
		contentView.findViewById(R.id.pop_group_chat).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				//发起群聊
				T.show(context, "暂未实现", Toast.LENGTH_SHORT);
				Intent intent=new Intent(mApplication, GroupChatActivity.class);
				context.startActivity(intent);
				popupwindow.dismiss();
			}
		});
		contentView.findViewById(R.id.pop_richscan).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				//扫一扫下载
				T.show(context, "扫一扫下载", Toast.LENGTH_LONG);
				Intent intent=new Intent(mApplication, UpdateActivity.class);
				context.startActivity(intent);
				popupwindow.dismiss();
			}
		});
		...
		// 隐藏对话框：点击返回按钮，点击窗口意外的区域 必须满足的2个条件：
		// 1.必须设置它的背景图片 2.设置可获取焦点
		popupwindow.setBackgroundDrawable(mApplication.getResources().getDrawable(R.drawable.skin_header_bar_bg));
		popupwindow.setFocusable(true);

		// 设置位置显示
		int[] location = new int[2];
		addItemBtn.getLocationInWindow(location);
		// x = location[0]; y = location[1];
		popupwindow.showAtLocation(addItemBtn, Gravity.TOP | Gravity.LEFT,
				location[0] + addItemBtn.getWidth() - popupwindow.getWidth(), location[1] + addItemBtn.getHeight());
