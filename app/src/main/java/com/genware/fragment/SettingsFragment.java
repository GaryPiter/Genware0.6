...
import com.genware.activity.AboutActivity;
import com.genware.activity.FeedBackActivity;
...
import com.genware.db.RosterDatabaseHelper;
import com.genware.db.RosterProvider;
import com.genware.service.XXService;
import com.genware.switcher.Switch;
...
public class SettingsFragment extends Fragment implements OnClickListener, OnCheckedChangeListener {
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		// 引用退出界面
		mExitConfirmBtn.setText("退出XMPP");
		mExitCancleBtn.setText("注销帐号");
	...
	public void readData() {
		...
		mStatusIcon.setImageResource(MainActivity.mStatusMap.get(PreferenceUtils.getPrefString(getActivity(),
				PreferenceConstants.STATUS_MODE, PreferenceConstants.AVAILABLE)));
		mStatusView.setText(PreferenceUtils.getPrefString(getActivity(), PreferenceConstants.STATUS_MESSAGE,
				getActivity().getString(R.string.status_available)));
		mNickView.setText(XMPPHelper
				.splitJidAndServer(PreferenceUtils.getPrefString(getActivity(), PreferenceConstants.ACCOUNT, "")));
		mShowOfflineRosterSwitch
				.setChecked(PreferenceUtils.getPrefBoolean(getActivity(), PreferenceConstants.SHOW_OFFLINE, true));

		mNotifyRunBackgroundSwitch
				.setChecked(PreferenceUtils.getPrefBoolean(getActivity(), PreferenceConstants.FOREGROUND, true));
		mNewMsgSoundSwitch
				.setChecked(PreferenceUtils.getPrefBoolean(getActivity(), PreferenceConstants.SCLIENTNOTIFY, false));
		mNewMsgVibratorSwitch
				.setChecked(PreferenceUtils.getPrefBoolean(getActivity(), PreferenceConstants.VIBRATIONNOTIFY, true));
		...
		mVisiableNewMsgSwitch
				.setChecked(PreferenceUtils.getPrefBoolean(getActivity(), PreferenceConstants.TICKER, true));
		mShowHeadSwitch
				.setChecked(PreferenceUtils.getPrefBoolean(getActivity(), PreferenceConstants.SHOW_MY_HEAD, true));
		mConnectionAutoSwitch
				.setChecked(PreferenceUtils.getPrefBoolean(getActivity(), PreferenceConstants.AUTO_RECONNECT, true));
		mPoweronReceiverMsgSwitch
				.setChecked(PreferenceUtils.getPrefBoolean(getActivity(), PreferenceConstants.AUTO_START, true));
		mSendCrashSwitch
				.setChecked(PreferenceUtils.getPrefBoolean(getActivity(), PreferenceConstants.REPORT_CRASH, true));
	}
	...
	public void logoutDialog() {
		new CustomDialog.Builder(getActivity()).setTitle(getActivity().getString(R.string.open_switch_account))
				.setMessage(getActivity().getString(R.string.open_switch_account_msg))
